param(
    ...
    [string]$PromptOut = "logs/prompts/claude-implementer-000_template.prompt.txt",
    [string]$LogOut = "logs/20260426-claude-implementer-000_template.json"
)
...
$log = [ordered]@{
    run_id = "20260426-claude-$Role-000_template"
    runner = "claude-dry-run"
    role = $Role
    task_id = $TaskId
    inputs = @(
        $Plan,
